...
class ReportesGastosService {
  /// Obtiene el reporte general de gastos por actividad para una semana específica (CORREGIDO - usa lógica del historial)
  Future<List<Map<String, dynamic>>> obtenerReporteGeneralPorSemana(
    ...
    try {
      print('📊 [REPORTES] Generando reporte para semana $semanaId...');
      
      // 🎯 SOLO BUSCAR EN TABLA SEMANAL (según solicitud del usuario)
      print('📊 [REPORTES] Buscando únicamente en nomina_empleados_semanal para semana $semanaId');
      ...
      print('📊 [REPORTES] Resultados encontrados: ${result.length}');
  ...
  /// Obtiene todas las semanas cerradas disponibles para reportes (SOLO TABLA SEMANAL)
  Future<List<Map<String, dynamic>>> obtenerSemanasDisponibles() async {
    final db = DatabaseService();
    await db.connect();
    
    try {
      // 🎯 BUSCAR SOLO SEMANAS CON DATOS EN nomina_empleados_semanal
      final result = await db.connection.query('''
        SELECT DISTINCT 
          s.id_semana,
          s.fecha_inicio,
      ...
      print('📊 [REPORTES] Semanas con datos en tabla semanal: ${result.length}');
      
      List<Map<String, dynamic>> semanasDisponibles = result.map((row) {
        final semanaId = row[0] as int;
        print('✅ [REPORTES] Semana $semanaId incluida (datos en tabla semanal)');
