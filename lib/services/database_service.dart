...
Future<List<Map<String, dynamic>>> obtenerSemanasCerradas() async {
  ...
  try {
    ...
    for (var row in results) {
      ...
      print(
        '🔍 [DEBUG] Procesando semana ID: $semanaId, fechas: $fechaInicio - $fechaFin',
      );
      ...
      print(
        '🔍 [DEBUG] Cuadrillas obtenidas para semana $semanaId: ${cuadrillasInfo.length}',
      );
      ...
      print(
        '🔍 [DEBUG] Total calculado para semana $semanaId: \$${totalSemana}',
      );
    ...
    await db.close();
    print(
      '🔍 [DEBUG] Procesamiento completado, retornando ${semanasCerradas.length} semanas',
    );
...
Future<List<Map<String, dynamic>>> obtenerCuadrillasDatosCompletos(
  int semanaId,
) async {
  final db = DatabaseService();
  ...
  try {
    ...
    print(
      '🔍🔍🔍 [HISTORIAL DEBUG] INICIANDO obtenerCuadrillasDatosCompletos para semana ID: $semanaId',
    );
    ...
    final totalRegistros =
        verificarNomina.isNotEmpty ? verificarNomina.first[0] : 0;
    print(
      '🔍🔍🔍 [HISTORIAL DEBUG] Total registros en nomina_empleados_semanal para semana $semanaId: $totalRegistros',
    );

    if (totalRegistros == 0) {
      print(
        '❌❌❌ [HISTORIAL DEBUG] No hay datos de nómina para semana $semanaId - RETORNANDO LISTA VACÍA',
      );
      await db.close();
      return [];
    }
    ...
    for (var reg in ejemploRegistros) {
      print(
        '   - ID Nómina: ${reg[0]}, Empleado: ${reg[1]}, Cuadrilla: ${reg[2]}, Total: ${reg[3]}, Neto: ${reg[4]}',
      );
    }
    ...
    print(
      '🔍🔍🔍 [HISTORIAL DEBUG] Encontradas ${cuadrillasResult.length} cuadrillas con empleados para semana $semanaId',
    );
    ...
    for (var cuadrillaRow in cuadrillasResult) {
      ...
      print(
        '🔍 [DEBUG] Procesando cuadrilla: $cuadrillaNombre (ID: $cuadrillaId)',
      );
      ...
      print(
        '🔍 [DEBUG] Encontrados ${empleadosResult.length} empleados en cuadrilla $cuadrillaNombre',
      );
      ...
      for (var empRow in empleadosResult) {
        ...
        print(
          '🔍 [DEBUG] Empleado ${empRow[1]}: total_neto=$totalNeto, total=$total, debe=$debe',
        );
        print(
          '🔍 [DEBUG] Días individuales: D1=${_parseDouble(empRow[3])}, D2=${_parseDouble(empRow[4])}, D3=${_parseDouble(empRow[5])}, D4=${_parseDouble(empRow[6])}, D5=${_parseDouble(empRow[7])}, D6=${_parseDouble(empRow[8])}, D7=${_parseDouble(empRow[9])}',
        );
    ...
    await db.close();
    print(
      '🔍 [DEBUG] Retornando ${cuadrillasInfo.length} cuadrillas con datos completos',
    );
...
Future<List<Map<String, dynamic>>> obtenerEmpleadosAsignadosCuadrilla(
  ...
  try {
    ...
    print(
      '❌ Error al obtener empleados asignados a cuadrilla $cuadrillaId: $e',
    );
    await db.close();
    return [];
  }
...
Future<Map<String, dynamic>> crearBackupSemana(
  int semanaId,
  ...
  try {
    ...
    try {
      ...
      print(
        '🔄 Iniciando backup para semana $semanaId con timestamp: $timestamp',
      );
      ...
      try {
        ...
        if (columnCheck.isEmpty) {
          ...
          await connection.execute(
            'ALTER TABLE backup_control ADD COLUMN archivo_backup VARCHAR(255)',
          );
        }
        ...
        if (rutaColumnCheck.isEmpty) {
          ...
          await connection.execute(
            'ALTER TABLE backup_control ADD COLUMN ruta_archivo TEXT',
          );
        }
        ...
        if (tamanoColumnCheck.isEmpty) {
          ...
          await connection.execute(
            'ALTER TABLE backup_control ADD COLUMN tamano_archivo BIGINT',
          );
        }
      } catch (alterError) {
      ...
      buffer.writeln(
        '-- Fecha: ${DateFormat('yyyy-MM-dd HH:mm:ss').format(now)}',
      );
      ...
      buffer.writeln(
        'DELETE FROM nomina_empleados_semanal WHERE id_semana = $semanaId;',
      );
      buffer.writeln('');
      ...
      for (final row in result) {
        ...
        buffer.writeln(
          'INSERT INTO nomina_empleados_semanal VALUES ($values);',
        );
      }
...
Future<Map<String, dynamic>> descargarBackup(
  String rutaArchivo,
  ...
  try {
    ...
    File archivoOrigen;
    if (rutaArchivo.startsWith('backups/') ||
        rutaArchivo.startsWith('backups\\')) {
    ...
    return {'success': false, 'error': 'Error al descargar archivo: $e'};
  }
}

/// Marca una semana como cerrada en la base de datos y guarda los datos en historial
Future<bool> cerrarSemanaEnBD(
  int idSemana, {
  String? autorizadoPor,
  List<Map<String, dynamic>>? datosNomina,
  bool mantenerCuadrillas = true,
}) async {
  print('🔥🔥🔥 [CERRAR SEMANA] ¡¡¡FUNCIÓN CERRAR SEMANA INICIADA!!! 🔥🔥🔥');
  print(
    '📋 [CERRAR SEMANA] Parámetro recibido - idSemana: $idSemana (tipo: ${idSemana.runtimeType})',
  );
  print(
    '👤 [CERRAR SEMANA] Usuario autorizado: ${autorizadoPor ?? 'NO PROPORCIONADO - ERROR'}',
  );
  ...
  try {
    ...
    print(
      '🔍 [CERRAR SEMANA] Query ejecutada, resultados: ${verificarCerrada.length}',
    );
    ...
    print(
      '🔍 [CERRAR SEMANA] Query de datos ejecutada, resultados: ${verificarDatos.length}',
    );
    ...
    try {
      ...
      if (autorizadoPor == null || autorizadoPor.trim().isEmpty) {
        throw Exception(
          'ERROR: Se requiere un usuario autorizado válido para cerrar la semana. No se puede usar "sistema" por defecto.',
        );
      }
      ...
      print(
        '✅ [CERRAR SEMANA] UPDATE ejecutado exitosamente, filas afectadas: $updateResult',
      );
    ...
    print(
      '🔍 [CERRAR SEMANA] Query de verificación ejecutada, resultados: ${verificarResultado.length}',
    );
    ...
    if (verificarResultado.isNotEmpty) {
      ...
      print(
        '   - esta_cerrada: $estaCerradaValor (tipo: ${estaCerradaValor.runtimeType})',
      );
      ...
      if (estaCerradaValor != true) {
        print(
          '❌ [CERRAR SEMANA] FALLA: esta_cerrada no es true, es: $estaCerradaValor',
        );
      }
      ...
      print(
        '❌ [CERRAR SEMANA] No se encontraron resultados en la verificación',
      );
    }

    final exitoso =
        verificarResultado.isNotEmpty && verificarResultado.first[0] == true;
    ...
    if (exitoso) {
      ...
      print(
        '� [CERRAR SEMANA] Copiando datos de nomina_empleados_semanal a nomina_empleados_historial...',
      );
      ...
      try {
        if (datosNomina != null && datosNomina.isNotEmpty) {
          ...
          print(
            '🔄 [HISTORIAL] Eliminando registros existentes para semana $idSemana...',
          );
          ...
          print(
            '🔄 [HISTORIAL] Insertando ${datosNomina.length} registros en historial...',
          );
          print(
            '🔍 [HISTORIAL DEBUG] Primer registro a insertar: ${datosNomina.first}',
          );
          ...
          for (var registro in datosNomina) {
            try {
              ...
              print(
                '❌ [HISTORIAL] Error al insertar registro ${registro['id_empleado']}: $insertError',
              );
          ...
          ''');
          print(
            '✅ [HISTORIAL] ¡DATOS GUARDADOS! $registrosInsertados/${datosNomina.length} registros insertados',
          );
          print(
            '✅ [HISTORIAL] VERIFICACIÓN: ${verificacion.first[0]} registros en historial para semana $idSemana',
          );
        } else {
          print(
            '⚠️ [HISTORIAL] No se proporcionaron datos de nómina para guardar en historial',
          );
        }
        print(
          '📋 Los datos de la semana $idSemana ya están disponibles para reportes',
        );

        // 6. LIMPIAR O MANTENER CUADRILLAS SEGÚN ELECCIÓN
        if (!mantenerCuadrillas) {
          print(
            '🔄 [LIMPIAR] Eliminando datos de nomina_empleados_semanal para la siguiente semana...',
          );
          try {
            await db.connection.execute('''
            ...
            ''');
            print(
              '✅ [LIMPIAR] Datos eliminados, cuadrillas reseteadas para la siguiente semana',
            );
          } catch (limpiarError) {
          ...
          print(
            '📋 [MANTENER] Manteniendo cuadrillas actuales para la siguiente semana',
          );
        }
      } catch (historialError) {
        print(
          '❌❌❌ [HISTORIAL] ERROR CRÍTICO al copiar a historial: $historialError',
        );
      ...
      print(
        '❌ [DEBUG] Valor de esta_cerrada después del UPDATE: ${verificarResultado.isNotEmpty ? verificarResultado.first[0] : 'SIN RESULTADOS'}',
      );
    }
    ...
    try {
      ...
      print(
        '❌ [CERRAR SEMANA] Error adicional al cerrar conexión: $closeError',
      );
    }
    return false;
  }
